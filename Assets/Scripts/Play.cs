...
sealed public class Play : MonoBehaviour {
    ...
    void Start()
    {
        ...
        /*if (LevelLoader.levelToLoad < 0)
            StartLevel(
            LevelLoader().JSONToLoad);
        else
        */
        //StartLevel(1);
        /*
        string rawJSON = Crypto.Decrypt(System.IO.File.ReadAllText("LevelData"));
        List<object> allLevels = Json.Deserialize(rawJSON) as List<object>;
        Dictionary<string, object> levelData = allLevels[2] as Dictionary<string, object>;
        LevelLoader.LoadFromJSON(levelData, -1, false);
        */
        //LevelLoader.levelToLoad = 3;
        //LevelLoader.Load("LevelData", LevelLoader.levelToLoad);
        StartLevel(3);
	}
