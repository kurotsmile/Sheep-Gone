...
sealed public class LevelEditor : MonoBehaviour
{
    ...
    private void PopulateLevelList()
    {
        ...
        box.create_btn_menu_header(game.iconImportData).set_act(() =>
        {
            game.file.Set_filter(Carrot_File_Data.JsonData);
            game.file.Open_file(s_path =>
            {
                string s_data = FileBrowserHelpers.ReadTextFromFile(s_path[0]);
                Dictionary<string, object> levelData = Json.Deserialize(s_data) as Dictionary<string, object>;
                if (levelData["name"] != null)
                {
                    if (box != null) Destroy(box.gameObject);
                    game.mLevel.Add(s_data);
                    LevelActionSelect("Load");
                    game.carrot.Show_msg("Import Level", "Level imported successfully!", Msg_Icon.Success);
                }
                else
                {
                    game.carrot.Show_msg("Import Level", "Level import fail!", Msg_Icon.Error);
                }
            });
        });
        ...
        for (int i = 0; i < listLevel.Count; i++)
        {
            ...
            item_lv.set_act(() =>
            {
                if (this.box != null) box.close();
                game.carrot.play_sound_click();
                StartCoroutine(LoadLevelWait(d));
            });

            Carrot_Box_Btn_Item btnExport = item_lv.create_item();
            btnExport.set_icon(game.iconExportData);
            btnExport.set_icon_color(Color.white);
            btnExport.set_color(game.carrot.color_highlight);
            btnExport.set_act(() =>
            {
                game.file.Set_filter(Carrot_File_Data.JsonData);
                game.file.Save_file(s_paths =>
                {
                    string s_data = Json.Serialize(d);
                    FileBrowserHelpers.WriteTextToFile(s_paths[0], s_data);
                    game.carrot.Show_msg("Export Level", "Level exported successfully!", Msg_Icon.Success);
                });
            });
